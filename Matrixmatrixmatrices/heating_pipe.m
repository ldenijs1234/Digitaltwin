...
function [h_outside,Q_in]  = heating_pipe(GH,T_in,T_air,T_pipe,humidity,co_2)
     %%inputs
    ...
    H_out = humidity; % Kg/m^3
    C_out = co_2; % Kg/m^3
    g = GH.p.Gravity;
    
    %%Calculated geometrical parameters of the pipe
    S = GH.p.Afin; % surface area of a fin in m^2
    ...
    epsil = M*sqrt((1-c)^3 /(2*log(1/c)));
    u = (2/3)*epsil*sqrt(2*log(1/c)/(1-c)^3);
    v = (2/3)*epsil*sqrt(2*c^3 *log(1/c)/(1-c)^3);

    Fin_efficiency = (sqrt(2*c*(1-c)^3)/(epsil*(1-c^2)))*((besseli(2/3,u)*besseli(-2/3,v)-besseli(-2/3,u)*besseli(2/3,v))...
    ...
    Vel_water = 1; %flow speed of the water
    m = Vel_water*A_in*water.density(T_in); %mass flow through the pipe in kg/s when the flow speed of the water is 1 m/s
