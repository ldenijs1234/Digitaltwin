...
thickness = 0.1;            % m (thickness of walls)
...
% Simulation loop
for t = 1:time_steps
    outside_temperature = (15 + 5*sin(2*pi*t/));   % degrees Celsius
    % Calculate temperature difference
    delta_temp = temperature(t) - outside_temperature;
    
    % Calculate energy balance
    % Heat loss due to conduction through the walls
    heat_loss_conduction_w = thermal_conductivity * area_walls * delta_temp / thickness;
    heat_loss_conduction_r = thermal_conductivity * area_roof * delta_temp / thickness;
    % Heat loss due to ventilation (assuming natural convection)
    % Calculating air changes per hour (ACH) for natural convection
    ACH = 0.005; % Example value (air changes per hour)
    air_volume = volume;
    air_flow_rate = ACH * air_volume / 3600; % m^3/s
    
    % Total heat loss
    total_heat_loss = heat_loss_conduction_r + heat_loss_conduction_w;
    
    % Calculate total heat input for the time step
    heating_input = heating_power_per_second * (3600 / time_steps); % watts
